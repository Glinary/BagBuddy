...
async function onload() {
  ...
  try {
    const { sched, collab } = await findBagDateCollab();
    ...
    if (collab.length >= 1) {
      if (collab.length < 3) {
        collabDiv.innerHTML += `<div class="inside-bag-round center hoverText"><span class="tooltip-text" id="top">${"you"}</span><img src="/static/images/boy.png" alt=""></div>`;
        for (i = 0; i < collab.length; i++) {
          collabDiv.innerHTML += `<div class="inside-bag-round center hoverText"><span class="tooltip-text" id="top">${collab[i].name}</span><img src="/static/images/boy.png" alt=""></div>`;
        }
      } else {
        let j = 0;
        collabDiv.innerHTML += `<div class="inside-bag-round center hoverText"><span class="tooltip-text" id="top">${"you"}</span><img src="/static/images/boy.png" alt=""></div>`;
        ...
        while (j < 1) {
          collabDiv.innerHTML += `<div class="inside-bag-round center hoverText"><span class="tooltip-text" id="top">${collab[j].name}</span><img src="/static/images/boy.png" alt=""></div>`;
          j++;
        }
...
async function findBagDateCollab() {
  ...
  if (resCode == 200) {
    ...
    return { sched: sched, collab: collab };
  } else {
    Swal.fire({
      position: "center",
      icon: "error",
