...
Valid Register
    ...
    Input Register      ${username}        ${email}      testing
    Click element       ${REG_BUTTON}
    Wait Until Element Is Visible    ${HOME_TITLE}

Invalid Username Existing
    Open Browser in Register Page
    Input Register      ${EXIST_USERNAME}        ${VALID_EMAIL}      ${VALID_PASSWORD}
    Click element       ${REG_BUTTON}
...
Invalid Email Input
    Open Browser in Register Page
    Input Register      ${EXIST_USERNAME}        ${INVALID_EMAIL}      ${VALID_PASSWORD}
    Click element       ${REG_BUTTON}
    ${text}=        Retrieve Error Message     id:register-email-feedback
